  ...
  async (req, res) => {
    ...
    try {
      ...
      if (group) {
        return res
          .status(400)
          .json({ errors: [{ msg: 'group with that name already exist' }] });
      }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!group) {
        return res
          .status(400)
          .json({ errors: [{ msg: 'group doesnt exist' }] });
      }
      ...
      for (let index = 0; index < group.followers.length; index++) {
        ...
        FollowerProfile.followingGroups =
          FollowerProfile.followingGroups.filter(
  ...
  async (req, res) => {
    try {
      ...
      if (!groups) {
        return res
          .status(400)
          .json({ errors: [{ msg: 'there are no groups for this user' }] });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (!group) {
        return res
          .status(400)
          .json({ errors: [{ msg: 'this group doesnt exist' }] });
      }
